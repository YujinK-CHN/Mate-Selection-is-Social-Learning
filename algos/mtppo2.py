                ...
                for i, task in enumerate(self.env.tasks): 
                    ...
                    for epoch in range(int((self.batch_size / len(self.env.tasks)) / self.max_cycles)): # num_cycles(20)
                        ...
                        for step in range(0, self.max_cycles): # 500
                            ...
                            rb_actions[i, index] = action
                            rb_logprobs[i, index] = logprob
                            rb_values[i, index] = value.flatten()
            ...
            for epoch in range(self.epoch_opt): # 16
                ...
                for start in range(0, rb_obs.shape[1], self.min_batch):
                    ...
                    for i, task in enumerate(self.env.tasks):
                        ...
                        if self.continuous == True:
                            old_actions = rb_actions[i, batch_index, :]
                        else:
                            old_actions = rb_actions.long()[i, batch_index, :]
                        _, newlogprob, entropy, values = self.policy.get_action_and_value(x = rb_obs[i, batch_index, :], action = old_actions)
                        ...
                        task_loss = pg_loss - self.ent_coef * entropy_loss + v_loss * self.vf_coef

                        loss[i] = task_loss
                    
                    loss = torch.sum((self.task_weights * loss), dim=-1)
